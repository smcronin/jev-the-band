...
## 2026-09-22 — Key variety across jams

**User observation:** almost every jam sounds like C dorian. **Evidence from the production archive (22 live songs):** tonic C in 9, E in 7, D in 3, only four of twelve tonics ever used; dorian in 9; both head-era songs were C dorian, one of them against a concept that said C♯. **Causes:** the arranger's priors (an LLM asked for a jam-band head reaches for C/D/E and dorian), examples in the head prompt all in D and a "C=0" pitch-class hint, Jev's opening decision copying the concept at p≈1 when one exists and preferring C or E when not, and nothing remembering previous jams.
