...
## 2026-09-22 — Key variety

- Production archive read over HTTPS: 22 live songs, tonics C 9 / E 7 / D 3 / A 1 / F 1, dorian 9, both head-era songs C dorian.
- `npm run check`: 121 tests pass, build clean. New tests cover resting tonics and modes, the opening menu, the arranger request, the archive's recent-key report from song rows and older opening keys, the off-model tonic (never a resting one, spread over at least eight tonics), the three-mode offer, head transposition by the shorter interval with drums untouched, and `|`-separated bars.
- Live `audit:head` runs with the archive's recent keys supplied: hint only — 1 of 3 heads left the resting tonics; room-chosen tonic — 3 of 3 written in the asked tonic (G, A, B) with no transposition, but all mixolydian; with modes resting — all lydian; with the three-mode offer — A minor, A harmonic minor, A melodic minor, A minor across four prompts (A was a 27% draw; a 2,000-draw sample of the chooser spreads over eight tonics). Zero unreadable events except one head that wrote twelve bars in one string, now handled.
- Not verified: listening, the transposition path against a real head, and the hosted deployment.
