...
import { noteNames, personas } from '../shared/music.js';
...
export async function composeHead(
  ...
  const tonic = concept ? noteNames[concept.root].replace('♯', '#').replace('♭', 'b') : undefined;
  const rootNames = noteNames.map((n) => n.replace('♯', '#').replace('♭', 'b'));
  const request = {
    model,
    temperature: 0.8,
    max_tokens: 6000,
  ...
  try {
    ...
    const { parts, dropped } = readHead(head);
